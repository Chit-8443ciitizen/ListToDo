...
const Week = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setTaskArray(tasks.filter(task => moment(task.task_deadline,'DD-MM-YYYY').isBetween(startDayWeek, endDayWeek, null, '[]') 
        || moment(task.task_deadline,'DD-MM-YYYY').isSame(startDayWeek, 'day')
        || moment(task.task_deadline,'DD-MM-YYYY').isSame(endDayWeek, 'day'))
        );
    } catch (err) {
      ...
      <div className="h-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-row items-center justify-between">
          <p className="text-gray text-start opacity-50 tracking-wider my-5">
            Start-week: {startDayWeekFormat}{" "} <br /> End-week: {endDayWeekFormat}
          </p>
          <CreateTask time={startDayWeekFormat} />
        </div>
