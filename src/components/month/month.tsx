...
const startDayMonthFormat = moment().startOf('month').add(0, 'days').format("DD-MM-YYYY");
const endDayMonthFormat = moment().endOf('month').add(0,'days').format("DD-MM-YYYY");

const Month = () => {

  const [taskArray, setTaskArray] = useState<Task[]>([]);
  const [listIdDel, setListIdDel] = useState<Array<string>>([]);
  const [clickBtnFilterWeek, setBtnFilterWeek] = useState<boolean>(false);
  ...
  const fetchData = async () => {
    try {
      ...
      const startDayMonth = moment().startOf('month').add(1,'days');
      const endDayMonth = moment().endOf('month').add(1,'days');
     
      setTaskArray(tasks.filter(task => moment(task.task_deadline,'DD-MM-YYYY').isBetween(startDayMonth, endDayMonth, null, '[]') 
        || moment(task.task_deadline,'DD-MM-YYYY').isSame(startDayMonth, 'day')
        || moment(task.task_deadline,'DD-MM-YYYY').isSame(endDayMonth, 'day'))
        );
    } catch (err) {
  ...
  const DisplayAll = () => {
      ...
      ( displayDeleted === true) &&  fetchData();
  };
  const DisplayNotDel = () => {
      setDisplayDeleted(displayDeleted => displayDeleted = false);
        ( displayDeleted === false) && fetchData();
      ...
      <div className="h-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-row items-center justify-between">
          <p className="text-gray text-start opacity-50 tracking-wider my-5">
            Start-month: {startDayMonthFormat}{" "} <br /> End-month: {endDayMonthFormat}
          </p>
          <CreateTask time={startDayMonthFormat} />
        </div>

        <button
          id="dropdownDefaultButton"
          onClick={()=>{ 
            if (clickBtnFilterWeek){setBtnFilterWeek(true)}
            else{setBtnFilterWeek(true)}
            }}
        ...
        <div
          id="dropdown"
          className={`z-10 ${clickBtnFilterWeek ? '' : 'hidden'} bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700`} >
          ...
          {taskArray.map((task) => (
            ...
            >
              ...
              <div className="">
                ...
                text-white bg-red-200 hover:bg-red-400 font-semibold duration-100 easy-in-out 
